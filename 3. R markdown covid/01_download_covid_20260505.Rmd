...
dir.create("../xlsx/covid", showWarnings = TRUE, recursive = FALSE, mode = "0777")
...
url_covid <- c("https://covid19.mhlw.go.jp/public/opendata/newly_confirmed_cases_daily.csv",
             "https://covid19.mhlw.go.jp/public/opendata/requiring_inpatient_care_etc_daily.csv",
             "https://covid19.mhlw.go.jp/public/opendata/deaths_cumulative_daily.csv",
             "https://covid19.mhlw.go.jp/public/opendata/severe_cases_daily.csv")
```
...
```{r}
file_covid <- url_covid %>% str_extract("(?<=\\/)[^\\/]+\\.csv")

path_covid <- paste0("../xlsx/covid/", file_covid)
```

```{r}
for (i in 1:length(url_covid)){
  download.file(url_covid[i], path_covid[i], mode = "wb")
  print(paste(file_covid[i], "downloaded"))
}
```
